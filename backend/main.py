...
# # --- PRODUCT ENDPOINTS ---
# # MODIFIED: This endpoint now fetches products from Sanity, not Supabase
...
# ):
#     """
#     Fetches products from Sanity CMS, optionally filtered by category slug.
#     """
#     raw_products = await fetch_all_products(category_slug=category) # Use new Sanity fetching function
...
#         # Sanity category is an object { _id, title, slug }
...
#             id=p.get('_id'), # Sanity's _id becomes the unified 'id'
#             slug=slug_value,
#             name=p.get('name'),
#             price=p.get('price'),
#             description=p.get('description'), # Portable Text can be 'Any'
#             category=category_title, # Map category title
#             imageUrl=p.get('imageUrl'), # Already a direct URL from Sanity service
#             alt=p.get('alt'),
#             stock=p.get('stock'),
...
# # <<<<< NEW ENDPOINT FOR FEATURED PRODUCTS (already using Sanity) >>>>>
...
#         category_title = product.get('category') # Already flattened to title by GROQ query
...
# # MODIFIED: This endpoint now fetches a single product from Sanity by slug
...
#     print(f"DEBUG (main.py): get_product endpoint received slug: '{product_slug}'")
#     if not product_slug or product_slug.lower() == "null":
#         print(f"DEBUG (main.py): Invalid or null slug received: '{product_slug}'")
#         raise HTTPException(status_code=400, detail="Invalid product slug")
#     # --- END DEBUGGING PRINTS ---

#     raw_product = await fetch_product_by_slug(product_slug) # Use new Sanity fetching function
    
#     # --- DEBUGGING PRINTS ADDED BELOW ---
#     print(f"DEBUG (main.py): Raw product data from fetch_product_by_slug: {raw_product}")
#     # --- END DEBUGGING PRINTS ---
...
#     # TRANSFORM SINGLE SANITY PRODUCT TO UNIFIED MODEL
...
async def get_products(
    ...
    Fetches products from Sanity CMS, optionally filtered by category slug.
    """
    print(f"DEBUG (main.py /products): Received category_slug: {category}")
    raw_products = await fetch_all_products(category_slug=category) 
    ...
    transformed_products = []
    for p in raw_products:
        print(f"DEBUG (main.py /products): Processing product in loop: {p}")
        print(f"DEBUG (main.py /products): Type of product in loop: {type(p)}")

        # MODIFIED: Slug is now a direct string from sanity_service.py's GROQ
        slug_value = p.get('slug') 
        
        # Category is a dereferenced object from fetch_all_products
        category_data = p.get('category')
        category_title = category_data.get('title') if isinstance(category_data, dict) else None

        transformed_products.append(ProductDisplayAPIModel(
            id=p.get('_id'), 
            slug=slug_value, # Now correctly extracted
            name=p.get('name'),
            price=p.get('price'),
...
async def get_featured_products_endpoint():
    ...
    """
    raw_products = await fetch_featured_products()
    print(f"DEBUG (main.py /products/featured): Raw products from Sanity service: {raw_products}")
    ...
    transformed_products = []
    for product in raw_products:
        print(f"DEBUG (main.py /products/featured): Processing product in loop: {product}")
        print(f"DEBUG (main.py /products/featured): Type of product in loop: {type(product)}")

        # MODIFIED: Slug is now a direct string from sanity_service.py's GROQ
        slug_value = product.get('slug') 
        
        # MODIFIED: Category is a direct string ("categoryTitle") from sanity_service.py's GROQ
        ...
        transformed_products.append(ProductDisplayAPIModel(
            ...
            price=product.get('price'),
            description=product.get('description'),
            category=category_title, # Now correctly extracted
            imageUrl=product.get('imageUrl'),
            alt=product.get('alt'),
            stock=product.get('stock'),
            isFeatured=product.get('isFeatured', False),
...
async def get_product(product_slug: str):
    ...
    Retrieves a single product by its slug from Sanity CMS.
    """
    print(f"DEBUG (main.py /products/{{slug}}): Received product_slug: '{product_slug}'")
    if not product_slug or product_slug.lower() == "null":
        print(f"DEBUG (main.py /products/{{slug}}): Invalid or null slug received: '{product_slug}'")
        raise HTTPException(status_code=400, detail="Invalid product slug")
    
    raw_product = await fetch_product_by_slug(product_slug) 
    print(f"DEBUG (main.py /products/{{slug}}): Raw product from Sanity service: {raw_product}")
    
    if not raw_product:
        print(f"DEBUG (main.py /products/{{slug}}): Product not found in Sanity service response for slug: {product_slug}")
        raise HTTPException(status_code=404, detail="Product not found")

    # MODIFIED: Slug is now a direct string from sanity_service.py's GROQ
    slug_value = raw_product.get('slug') 
    
    # Category is a dereferenced object from fetch_product_by_slug
    category_data = raw_product.get('category')
    category_title = category_data.get('title') if isinstance(category_data, dict) else None
