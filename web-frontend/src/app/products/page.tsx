    ...
    async function fetchPageData() {
      ...
      try {
        ...
        const fetchedProducts = await getProducts(
          currentCategorySlug || undefined,
          currentSortOrder,
          currentMinPrice ? parseFloat(currentMinPrice) : undefined,
          currentMaxPrice ? parseFloat(currentMaxPrice) : undefined
        );
        // IMPORTANT: Filter out any null or undefined products before setting state
        // This also helps prevent issues with missing 'id' for keys
          ...
          {products.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {products.map((product, index) => { // Added 'index' here
                // --- DEBUGGING: Log product ID and name for key prop ---
                if (!product.id) {
                ...
                // --- END DEBUGGING ---
                return (
                  <ProductCard key={product.id || `product-${index}`} product={product} /> // Added fallback key
                );
              })}
            </div>
          )}
